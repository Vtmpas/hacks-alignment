...
class JsonformerVLLM:
    ...
    def generate_number(self, temperature: Union[float, None] = None, iterations=0):
        ...
        response = self.llm.generate(
            prompt,
            SamplingParams(max_tokens=self.max_number_tokens,
    ...
    def generate_boolean(self) -> bool:
        ...
        true_match, mtv = np.argmax(np.where(logits == true_token_id, 1, 0)), np.max(np.where(logits == true_token_id, 1, 0))
        false_match, mfv = np.argmax(np.where(logits == false_token_id, 1, 0)), np.max(np.where(logits == false_token_id, 1, 0))
